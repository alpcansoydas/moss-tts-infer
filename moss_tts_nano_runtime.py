...
class NanoTTSService:
    ...
    @staticmethod
    def _print_generation_step(stage: str, details: str = "") -> None:
        message = f"[NanoTTS][{stage}]"
        if details:
    ...
    def synthesize(
        ...
        *,
        ...
        self._print_generation_step("START", "buffered synthesis request received")
        normalized_text = str(text or "").strip()
        if not normalized_text:
        ...
        self._print_generation_step(
            "VALIDATE",
            f"mode={normalized_mode} text_chars={len(normalized_text)}",
        )
        ...
        self._print_generation_step(
            "INPUTS",
            f"voice={resolved_voice} prompt_audio={'yes' if effective_prompt_audio_path else 'no'}",
        )
        ...
        self._print_generation_step("OUTPUT", f"path={output_path}")

        started_at = time.monotonic()
        with self._lock:
            self._print_generation_step("MODEL", "loading model + tokenizer")
            model = self._load_model_locked()
            ...
            self._print_generation_step(
                "MODEL_READY",
                (
                    f"attn_request={requested_attn_implementation} "
                    f"global={effective_global_attn_implementation} "
                    f"local={effective_local_attn_implementation}"
                ),
            )
            if seed is not None:
                ...
                self._print_generation_step("SEED", f"value={seed}")

            try:
                self._print_generation_step(
                    "INFERENCE",
                    (
                        f"max_new_frames={int(max_new_frames)} "
                        f"tts_batch={int(tts_max_batch_size)} codec_batch={int(codec_max_batch_size)}"
                    ),
                )
                ...
                self._print_generation_step(
                    "INFERENCE_DONE",
                    (
                        f"chunks={len(result.get('voice_clone_text_chunks') or [])} "
                        f"elapsed={time.monotonic() - started_at:.2f}s"
                    ),
                )
            except Exception:
                self._print_generation_step("ERROR", "inference failed; discarding loaded model/tokenizer")
        ...
        self._print_generation_step(
            "DONE",
            f"audio_saved={output_path} sample_rate={int(result['sample_rate'])}",
        )
        return {
    ...
    def synthesize_stream(
        ...
        *,
        ...
        self._print_generation_step("START", "streaming synthesis request received")
        normalized_text = str(text or "").strip()
        if not normalized_text:
            raise ValueError("text is required")
        ...
        with self._lock:
            ...
            try:
                self._print_generation_step(
                    "INFERENCE",
                    (
                        f"stream=true max_new_frames={int(max_new_frames)} "
                        f"tts_batch={int(tts_max_batch_size)} codec_batch={int(codec_max_batch_size)}"
                    ),
                )
                for event in model.inference_stream(
                    ...
                    if event_type == "audio":
                        self._print_generation_step(
                            "AUDIO_CHUNK",
                            (
                                f"index={int(event.get('chunk_index', 0))} "
                                f"emitted={float(event.get('emitted_audio_seconds', 0.0)):.2f}s "
                                f"lead={float(event.get('lead_seconds', 0.0)):.2f}s"
                            ),
                        )
                    ...
                    if event_type == "result":
                        self._print_generation_step("INFERENCE_DONE", "stream reported final result")
                        final_result = dict(event)
            except Exception:
                self._print_generation_step("ERROR", "streaming inference failed; discarding loaded model/tokenizer")
                self._discard_loaded_audio_tokenizer_locked(
        ...
        self._print_generation_step(
            "DONE",
            f"audio_saved={final_result['audio_path']} sample_rate={int(final_result['sample_rate'])}",
        )
        yield {
            "type": "result",
